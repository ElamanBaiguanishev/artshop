...
Хостинг - **Dokploy** поверх Docker на VPS.

### Два приложения, а не четыре

```
dokploy/
├── Dockerfile.backend          один образ на api + worker + chat-gateway
├── Dockerfile.web              Next в режиме standalone
├── docker-compose.backend.yml  Dokploy-приложение artshop-backend
└── docker-compose.web.yml      Dokploy-приложение artshop-web (сюда вешается домен)
```

Три роли бэкенда поднимаются **из одного образа**, различаясь только командой запуска.
Это тот же приём, что применяется в рабочих проектах автора: один деплой - много ролей.

Что это даёт:

- сборка одна вместо трёх - существенно на слабой VPS;
- роли всегда одной версии, рассинхрона схемы БД между `api` и `worker` не бывает;
- в Dokploy два приложения вместо четырёх.

### Что где живёт

| Компонент | Где |
|---|---|
| `api`, `worker`, `chat-gateway` | `artshop-backend`, один образ, три команды |
| `web` | `artshop-web`, на него вешается домен через Traefik |
| PostgreSQL | **отдельный сервис в Dokploy** - пересборка приложения не должна трогать боевую базу |
| Redis | внутри compose бэкенда, только под очередь |
| MinIO | внутри compose бэкенда, том на диске VPS |

Подключение к внешней сети `dokploy-network` с алиасами, labels `metrics.scrape`
для коллектора и `autoheal`, healthcheck на каждом сервисе - по образцу уже
работающих сервисов автора.

### Локальная разработка - это другое

`docker-compose.yml` в корне репозитория поднимает postgres, redis и minio **только
для локальной работы**, на нестандартных портах (5433, 6380), чтобы не конфликтовать
с уже запущенными на машине. Деплоем он не занимается - для этого файлы в `dokploy/`.

### Хранилище изображений

На старте - MinIO контейнером, данные на диске VPS. Код пишется под S3 API, поэтому
переезд на внешнее хранилище (Cloudflare R2, Object Storage) - это смена трёх
переменных окружения без единой строчки кода.

**Данные на диске VPS требуют бэкапа.** Фотографии - самое ценное в проекте:
каталог можно набрать заново, снимки работ, которые уже проданы и отправлены, - нет.
...
| монорепо | репозиторий на сервис | разработчик один, релиз всегда общий; схема БД и zod-контракты общие - смена поля ломает типы сразу в обоих концах, а не в рантайме на проде |
| одна сборка бэкенда на три роли | образ на каждый сервис | сборка одна вместо трёх, роли всегда одной версии; приём уже проверен в рабочих проектах |
| BullMQ + Redis | pg-boss (очередь в Postgres) | pg-boss убрал бы Redis и outbox-релей, но нишевый и незнаком; BullMQ уже в проде у автора и узнаваем в Node-вакансиях - платить изучением нишевой библиотеки не за что |
| BullMQ, а не RabbitMQ | брокер сообщений | Rabbit не даёт атомарности с БД, то есть outbox остался бы всё равно, плюс контейнер с Erlang ради маршрутизации, которой в проекте нет |
| внутренние пакеты без сборки | сборка в dual-package | `api` живёт в CommonJS (Nest, декораторы), остальное в ESM; отдавая исходники, каждое приложение компилирует под свой формат и конфликт форматов исчезает |
| MinIO на старте | сразу внешнее S3 | код одинаковый, переезд стоит трёх переменных окружения |
